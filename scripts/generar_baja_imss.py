...
LOG_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs', 'tareas_baja_imss.log')
...
BEGIN
    INSERT INTO tareas_gestion_talento (
        ...
        source, metadata_json
              ...
              NULL, NULL, NULL, 'AUTO_BAJA_IMSS', ?);
END
"""

QUERY = """
SELECT e.id AS employee_id, e.employee_number,
       jh.termination_date, COALESCE(gi.full_name, '') AS full_name
FROM employees e
...
def run():
    log.info('==== INICIO TAREAS BAJA IMSS ====')
    today = date.today()
    ...
    for row in rows:
        emp_id, emp_num, term_raw, full_name = row

        if isinstance(term_raw, datetime):
            term_date = term_raw.date()
        elif isinstance(term_raw, date):
            term_date = term_raw
        else:
            try:
                term_date = date.fromisoformat(str(term_raw)[:10])
            except Exception:
                invalid += 1
                continue

        due_date  = add_business_days(term_date, 5)
        bucket    = get_week_bucket(due_date, today)
        term_str  = str(term_date)
        ...
        metadata    = json.dumps({'rule': 'BAJA_IMSS_5_DIAS_HABILES', 'generated_at': datetime.now().isoformat(),
                                  'termination_date': term_str}, ensure_ascii=False)

        try:
            c = conn.cursor()
            c.execute(UPSERT_SQL, (
                'BAJA_IMSS', emp_id, term_str,
                due_str, bucket, emp_name, description, metadata,
                'BAJA_IMSS', emp_id, term_str,
                'BAJA_IMSS', title, description, emp_id, emp_num, emp_name,
                term_str, due_str, bucket, metadata,
            ))
            conn.commit()
            c.close()
            upserted += 1
            buckets[bucket].append({'employee_id': emp_id, 'employee_number': emp_num,
                                    'employee_name': emp_name, 'termination_date': term_str,
                                    'due_date': due_str})
    ...
    conn.close()
    log.info('Filas: %d | Invalidos: %d | Errores: %d | Upserted: %d', len(rows), invalid, errors, upserted)
    for b, lst in buckets.items():
        log.info('  %s: %d', b, len(lst))
    log.info('==== FIN TAREAS BAJA IMSS ====')
    return buckets
