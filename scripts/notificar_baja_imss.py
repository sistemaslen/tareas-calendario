...
LOG_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs', 'tareas_baja_notificacion.log')
logging.basicConfig(
...
def run():
    log.info('==== INICIO NOTIFICACION BAJA IMSS ====')
    conn = get_connection()
    ...
    log.info('==== FIN NOTIFICACION BAJA IMSS ====')
    conn.close()
