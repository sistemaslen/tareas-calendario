...
LOG_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs', 'tareas_firma_contrato_notificacion.log')
logging.basicConfig(
...
TASK_CODE = 'FIRMA_CONTRATO'


def run():
    log.info('==== INICIO NOTIFICACION FIRMA CONTRATO ====')
    conn = get_connection()
    ...
    log.info('==== FIN NOTIFICACION FIRMA CONTRATO ====')
    conn.close()
