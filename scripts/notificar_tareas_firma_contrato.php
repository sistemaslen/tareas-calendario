...
function log_msg($msg) {
    ...
    file_put_contents(__DIR__ . '/../logs/tareas_firma_contrato_notificacion.log', $line, FILE_APPEND);
}
...
function loadTasks($sqlsrv): array {
    $sql = "
    SELECT id, employee_number, employee_name, hire_date AS firma_date, due_date, week_bucket, status
    FROM tareas_gestion_talento
    WHERE task_code = 'FIRMA_CONTRATO'
    ...
    while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
        if ($row['firma_date'] instanceof DateTimeInterface) $row['firma_date'] = $row['firma_date']->format('Y-m-d');
...
function buildHtml(array $vencidas, array $actuales, array $proximas): string {
    ...
    $renderRows = function(array $items, string $emptyMsg) {
        ...
        $rows = '';
        foreach ($items as $t) {
            $firmaFormatted = !empty($t['firma_date']) ? date('d/m/Y', strtotime((string)$t['firma_date'])) : '-';
            ...
            $rows .= '<tr>'
                ...
                . '<td style="padding:10px 14px;border-bottom:1px solid #f3f4f6;font-size:13px;color:#6b7280;">' . $firmaFormatted . '</td>'
    ...
    $thStyle = 'padding:8px;';
    $tableStyle = 'width:100%;cellpadding:0;cellspacing:0;border-collapse:collapse;border:1px solid #e5e7eb;';

    $headerRow = '<thead><tr style="background:#f9fafb;">'
        . '<th style="' . $thStyle . '">No. Empleado</th>'
        . '<th style="' . $thStyle . '">Nombre</th>'
        . '<th style="' . $thStyle . '">Fecha firma contrato</th>'
    ...
    return '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"></head>'
        ...
        . '<tr><td><h2 style="margin:0;color:#1f1754;">Resumen semanal - Firma de contrato definitivo</h2>'
        . '<p style="color:#6b7280;">Generado el ' . $today . '</p></td></tr>'
        . '<tr><td><p style="color:#374151;">Seguimiento de firmas de contrato indeterminado/determinado (dia 57 desde el ingreso).</p></td></tr>'
        ...
        . $headerRow . '<tbody>' . $renderRows($vencidas, 'Sin tareas vencidas') . '</tbody></table></td></tr>'

        . '<tr><td><h3>Esta semana (' . $totalActuales . ')</h3></td></tr>'
        . '<tr><td><table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #e5e7eb;">'
        . $headerRow . '<tbody>' . $renderRows($actuales, 'Sin tareas para esta semana') . '</tbody></table></td></tr>'

        . '<tr><td><h3>Proxima semana (' . $totalProximas . ')</h3></td></tr>'
        . '<tr><td><table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #e5e7eb;">'
        . $headerRow . '<tbody>' . $renderRows($proximas, 'Sin tareas para la proxima semana') . '</tbody></table></td></tr>'
...
try {
    ...
    $to = findRecipients($sqlsrv, 'FIRMA_CONTRATO');

    if (empty($to)) {
        throw new RuntimeException('No hay destinatarios para FIRMA_CONTRATO.');
    }

    $subject = 'Tareas semanales de Gestion de Talento - Firma de contrato definitivo';
    ...
    log_msg('Notificacion semanal enviada para FIRMA_CONTRATO.');
