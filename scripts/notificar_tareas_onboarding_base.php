...
function log_msg($msg) {
    ...
    file_put_contents(__DIR__ . '/../logs/tareas_onboarding_base_notificacion.log', $line, FILE_APPEND);
}
...
function loadTasks($sqlsrv): array {
    $sql = "
    SELECT id, employee_number, employee_name, hire_date AS range_start, due_date, week_bucket, status
    FROM tareas_gestion_talento
    WHERE task_code = 'ONBOARDING_BASE'
    ...
    while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
        if ($row['range_start'] instanceof DateTimeInterface) $row['range_start'] = $row['range_start']->format('Y-m-d');
...
function buildHtml(array $vencidas, array $actuales, array $proximas): string {
    ...
    $renderRows = function(array $items, string $emptyMsg) {
        ...
        $rows = '';
        foreach ($items as $t) {
            $startFormatted = !empty($t['range_start']) ? date('d/m/Y', strtotime((string)$t['range_start'])) : '-';
            ...
            $rows .= '<tr>'
                ...
                . '<td style="padding:10px 14px;border-bottom:1px solid #f3f4f6;font-size:13px;color:#6b7280;">' . $startFormatted . '</td>'
    ...
    return '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"></head><body style="font-family:Segoe UI,Tahoma,Arial,sans-serif;background:#f1f0fb;padding:24px;">'
        ...
        . '<tr><td><h2 style="margin:0;color:#1f1754;">Resumen semanal - Onboarding Base</h2><p style="color:#6b7280;">Generado el ' . $today . '</p></td></tr>'
        . '<tr><td><p style="color:#374151;">Seguimiento onboarding para colaboradores BASE con contrato INDETERMINADO y estatus ALTA. Rango operativo: dia 7 al 10 desde ingreso.</p></td></tr>'
        ...
        . '<thead><tr style="background:#f9fafb;"><th style="padding:8px;">No. Empleado</th><th style="padding:8px;">Nombre</th><th style="padding:8px;">Inicio onboarding (dia 7)</th><th style="padding:8px;">Fecha limite (dia 10)</th><th style="padding:8px;">Estatus</th></tr></thead><tbody>' . $renderRows($vencidas, 'Sin tareas vencidas') . '</tbody></table></td></tr>'
        . '<tr><td><h3>Esta semana (' . $totalActuales . ')</h3></td></tr>'
        . '<tr><td><table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #e5e7eb;">'
        . '<thead><tr style="background:#f9fafb;"><th style="padding:8px;">No. Empleado</th><th style="padding:8px;">Nombre</th><th style="padding:8px;">Inicio onboarding (dia 7)</th><th style="padding:8px;">Fecha limite (dia 10)</th><th style="padding:8px;">Estatus</th></tr></thead><tbody>' . $renderRows($actuales, 'Sin tareas para esta semana') . '</tbody></table></td></tr>'
...
try {
    ...
    $to = findRecipients($sqlsrv, 'ONBOARDING_BASE');

    if (empty($to)) {
        throw new RuntimeException('No hay destinatarios para ONBOARDING_BASE.');
    }

    $subject = 'Tareas semanales de Gestion de Talento - Onboarding Base';
    ...
    log_msg('Notificacion semanal enviada para ONBOARDING_BASE.');
