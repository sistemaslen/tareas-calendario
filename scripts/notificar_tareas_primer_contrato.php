...
function log_msg($msg) {
    ...
    file_put_contents(__DIR__ . '/../logs/tareas_primer_contrato_notificacion.log', $line, FILE_APPEND);
}
...
function loadTasks($sqlsrv): array {
    ...
    WHERE task_code = 'PRIMER_CONTRATO'
...
function buildHtml(array $vencidas, array $actuales, array $proximas): string {
    ...
    return '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"></head><body style="font-family:Segoe UI,Tahoma,Arial,sans-serif;background:#f1f0fb;padding:24px;">'
        ...
        . '<tr><td><h2 style="margin:0;color:#1f1754;">Resumen semanal - Primer contrato</h2><p style="color:#6b7280;">Generado el ' . $today . '</p></td></tr>'
        . '<tr><td><p style="color:#374151;">Seguimiento de tareas de primer contrato (evento de un solo dia).</p></td></tr>'
        ...
        . '<thead><tr style="background:#f9fafb;"><th style="padding:8px;">No. Empleado</th><th style="padding:8px;">Nombre</th><th style="padding:8px;">Fecha ingreso</th><th style="padding:8px;">Fecha fin 1er contrato</th><th style="padding:8px;">Estatus</th></tr></thead><tbody>' . $renderRows($vencidas, 'Sin tareas vencidas') . '</tbody></table></td></tr>'
        . '<tr><td><h3>Esta semana (' . $totalActuales . ')</h3></td></tr>'
        . '<tr><td><table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #e5e7eb;">'
        . '<thead><tr style="background:#f9fafb;"><th style="padding:8px;">No. Empleado</th><th style="padding:8px;">Nombre</th><th style="padding:8px;">Fecha ingreso</th><th style="padding:8px;">Fecha fin 1er contrato</th><th style="padding:8px;">Estatus</th></tr></thead><tbody>' . $renderRows($actuales, 'Sin tareas para esta semana') . '</tbody></table></td></tr>'
        . '<tr><td><h3>Proxima semana (' . $totalProximas . ')</h3></td></tr>'
        . '<tr><td><table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;border:1px solid #e5e7eb;">'
        . '<thead><tr style="background:#f9fafb;"><th style="padding:8px;">No. Empleado</th><th style="padding:8px;">Nombre</th><th style="padding:8px;">Fecha ingreso</th><th style="padding:8px;">Fecha fin 1er contrato</th><th style="padding:8px;">Estatus</th></tr></thead><tbody>' . $renderRows($proximas, 'Sin tareas para la proxima semana') . '</tbody></table></td></tr>'
        . '</table></body></html>';
}
...
try {
    ...
    $to = findRecipients($sqlsrv, 'PRIMER_CONTRATO');

    if (empty($to)) {
        throw new RuntimeException('No hay destinatarios para PRIMER_CONTRATO.');
    }

    $subject = 'Tareas semanales de Gestion de Talento - Primer contrato';
    ...
    log_msg('Notificacion semanal enviada para PRIMER_CONTRATO.');
